...
遥测开放接缝；iPad 为**对等**设备）。
2026-08-04 — **[Amendment F](./amendments/F-2026-08-04-unreachable-surface.md)**
（实现 #35 前的规格核查发现的**缺口**：不可达状态的**告知位置**从未被规定过——
FR-053 只覆盖会话内的输入框，而**一台主机可能一个会话都没有**。补 FR-060
（host 列表 MUST 可见，证书类原因 MUST 可区分）与 FR-061（`certificateRejected`
与 `certificateChanged` 是因果不是同义，MUST NOT 合并））。
标 **[A]** / **[B]** / **[C]** / **[F]** 的内容分别由对应修正案引入或修改；理由与被否决的备选方案见修正案本身。
...
- **[F] FR-060**: 主机的**不可达状态与原因** MUST 在 **host 列表**可见，
  且**证书类原因 MUST 与其它原因可区分**（不得与「关机 / 网络不通」共用一句文案）。

  FR-053 覆盖的是**会话内**的输入框，而**一台主机可能一个会话都没有**——
  刚配对完就换了证书是最典型的情形，也恰恰是最需要告知的情形（配对刚发生，
  用户对「我刚连的这台机器身份变了」最有判断力）。宪法 V 规定了「不匹配即拒连、
  不弹继续信任」，但没规定在哪告知；这条补上告知位置。host 列表是唯一能覆盖
  「没有任何会话的主机」的挂载点。

- **[F] FR-061**: `HostUnreachableReason.certificateRejected` 与
  `HostPairingState.certificateChanged` **MUST NOT 合并**，且 host 行显示的是
  **后者**。

  两者不是同义词，是因果：前者是**一次连接尝试的结果**（派生态、不持久化，
  见本文件「Host 的派生态」一行 **[B]**），后者是**配对关系的状态**
  （持久，需要人介入才能离开）。一次 `certificateRejected` MUST **导致**
  `certificateChanged`，而不是与之并列显示——否则同一件事在 UI 上说两遍，
  且瞬时值会在下一次探测后消失，把一个需要人介入的状态显示成一闪而过的错误。
