...
**首次都有真实实现**后的双向比对 + 端到端实测）**：修正**契约自身**的五处缺陷——
...
**首次对真 bridge 的端到端联调**）**：澄清配对生命周期的四处——`invalid_token` 与
`token_revoked` 的清凭据分界（散文与错误码表此前互相矛盾，表已收窄）、token MUST
跨 bridge 重启持久化（转写遗漏）、配对请求本身就在已 pin 的连接上发出（不存在
「首次连接无 pin」这一步）、鉴权先于路由使 404/405 仅对已鉴权请求可达（有意，非缺陷）。
**四条都是把两侧已有的理解写下来，线上字节一字不变，
协议版本仍为 `1`**。标 **[E]**。
...
bridge **MUST** 在响应头 `x-localis-turn-id` 中给出 `turn_id`——
**便于客户端在收到任何 body 之前就记下它**。

> **[D] 此处原有第二个 MUST「在流的首个事件中回传 `turn_id`」，已删除**
> （[Amendment D](../amendments/D-2026-08-04-contract-gaps.md) §5b，2026-08-04）。
> 它**没有生产方、没有消费方，而且不解决它自己声称要解决的问题**——
> 括号里那个理由（「在收到任何 body 之前就记下它」）恰恰只有**响应头**能满足：
> **首个事件在 body 里，比响应头晚到。** 保留它只会让下一个读契约的人去实现一个
> 无人消费的字段，还以为自己在补缺口。**turn 身份的权威来源是响应头，唯此一处。**
>
> `turn_id` **仍**出现在 `x-localis-turn-end`（§3.1d）的载荷里——那是**终结事件**的
> 一部分，与本条无关，**不受影响**。
...
| **[E]** 404 | `not_found` | 路径不存在。**仅对已鉴权请求可达**（见下） |
| **[E]** 405 | `method_not_allowed` | 方法不匹配。**仅对已鉴权请求可达**（见下） |

**[E] 鉴权先于路由：未携带有效 token 的请求一律先得 `401`，不区分路径是否存在。**
这是有意的，不是遮蔽缺陷——否则未鉴权调用方可以用状态码差异探出路由表。
已鉴权请求得到的是真实的 `404` / `405`（2026-08-04 对真 bridge 实测确认：带有效
token 时 `GET /nope` → 404、`GET /localis/v1/pair` → 405；同样两条不带 token 时
均为 401）。

因此「路径笔误伪装成 token 被拒」这一失败模式**只在手工探测时出现**（curl 忘带
header），生产客户端配对后始终携带 token，不受影响。
...
- [ ] **[D]** `turn_id` 的权威来源是**响应头** `x-localis-turn-id`：**在收到任何 body 之前**
      即可记下它并据此做 turn 身份校验。**首个事件不再被要求携带 `turn_id`**——
      流的首个事件**不带** `turn_id` 时，客户端 **MUST** 照常工作（`x-localis-turn-end`
      载荷里的 `turn_id` 不受影响，仍照常解析）
